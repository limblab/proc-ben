function out = processXDS(filename,signal_info)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % this is a direct adaptation of Raeed's 'processCDS' function for XDS.
    %
    % loads an XDS file and returns all continuous signals it can extract, along
    ...
    % first the events...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % create a complete trial table to mimic CDS trials format.
    % additionally, when using cell2table, it keeps the result field as a
    % cell array in the table for whatever reason, so convert the cell array
    % into an ordinary string array

    trials = cell2table(xds.trial_info_table, 'VariableNames', {xds.trial_info_table_header{1:size(xds.trial_info_table, 2)}});
    trials.result = cell2mat(trials.result);
    
    event_names = trials.Properties.VariableNames(endsWith(trials.Properties.VariableNames,'Time'));
    ...
    for eventnum = 1:length(event_names)
        event_data{eventnum} = trials.(event_names{eventnum});
    end
    ...
    signal_names = {};
    if xds.meta.hasKinematics
        signal_names = [signal_names {'kin'}];
        kin = array2table([xds.kin_p xds.kin_v xds.kin_a], 'VariableNames', {'x' 'y' 'vx' 'vy' 'ax' 'ay'}); %create a unified kin table with labels if XDS has kinematics
    end
    if xds.meta.hasForce
        signal_names = [signal_names {'force'}];
        force = array2table(xds.force, 'VariableNames', {'fx' 'fy'}); %create a unified force table with labels if XDS has force
    end
    if xds.meta.hasEmg
    ...
    if xds.meta.hasAnalog
        % analog could be a lot of stuff...
        for analog_idx = 1:length(xds.analog)
            header = xds.analog{analog_idx}.Properties.VariableNames;
    ...
    for signum = 1:length(signal_names)
        switch lower(signal_names{signum})
            case 'kin'
                % do kin stuff
                data_table = kin;
                data_table = [data_table array2table(xds.time_frame, 'VariableNames', {'t'})];
                data_cols = startsWith(data_table.Properties.VariableNames,{'x','y','vx','vy','ax','ay'});
            case 'force'
                % do force stuff
                data_table = force;
                data_table = [data_table array2table(xds.time_frame, 'VariableNames', {'t'})];
                data_cols = startsWith(data_table.Properties.VariableNames,{'fx','fy'});
            case 'emg'
    ...
    meta_info = struct(...
        ...
        'task',xds.meta.task,...
        'date_time',xds.meta.dateTime,...
        'trialID',trials.number',...
        'result',trials.result'...
        );
    
    % import the trial table meta info
    for i=1:length(trial_meta)
        if ischar(trial_meta{i}) && ismember(trial_meta{i},trials.Properties.VariableNames)
            meta_info.(trial_meta{i}) = trials.(trial_meta{i})';
        else
