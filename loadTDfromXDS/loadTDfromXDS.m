function trial_data = loadTDfromXDS(filename,params)
    ...
    %               to be loaded in from XDS trial table (names should be desired column names of trial table)
    ...
    %               the XDS, so you don't need to add them here.
    ...
    if isempty(xds_array_name)
        warning('XDS array name not provided')
    end
    ...
    if ~isempty(xds_array_name)
        if ~iscell(xds_array_name) && ~isempty(xds_array_name)
            xds_array_name = {xds_array_name};
        end
        assert(ischar(xds_array_name{1}),'xds_array_name must contain a string')
        
        assert(length(array_name)==length(xds_array_name),'Length of array_name must match length of xds_array_name (if provided)')
    else
    ...
    spike_routine = @processXDSspikes;
    xds_routine = @processXDS;
    ...
    for arraynum = 1:length(array_name)
        signal_info{arraynum} = initSignalStruct( ...
            ...
            'params',struct('xds_array_name',xds_array_name{arraynum}), ... 
...
function names = get_marker_labels()
    % function to return the names of markers, as they appear in raw xds format
